...
- For the Operating Layer installer surface, treat the unified JSON install package as the installation SSOT.
  - Primary install artifact: `Helikon-mini_Install_Package_v3.0.6.json`
  - Human-readable projection/fallback: `Helikon-mini_OPERATING_LAYER_v3.0.6_install.md`
...
# Helikon-mini Operating Layer v3.0.6 — install (Saved Memories pack)

This file preserves the JSON-first install surface, normalized installer command family, and `APPROVE` build token while advancing the synchronized package pointer to the v3.0.6 SETUP-clarity + synchronized-surface closure set and leaving payload text and runtime semantics unchanged.

**Memory content version:** v3.0.0.  
**Install surface version:** v3.0.6.
